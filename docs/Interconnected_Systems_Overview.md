...
Crucially, variants do not overwrite the base; they coexist as parallel interpretations. On disk, each variant lives under `sources/characters/<slug>/variants/<variant_name>/`, anchored by a delta-only `spec_v2_fields.md` generated from staging draft prompts (`sources/staging_drafts.md`, `###` group + `####` variant). This allows users, at consumption time, to select the version of the character that best matches their desired tone, relationship state, or narrative context, while preserving consistency and traceability across all versions. `seed_phrase.txt` is deprecated for this workflow and no longer required.
